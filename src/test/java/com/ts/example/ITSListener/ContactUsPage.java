...
public class ContactUsPage extends ExampleBasePage {
	...
	*/ 
	...
	public Validator verify(){
		return new Validator() {
			...
			@Override
			public void Validate() {
				//Assert.assertEquals(getPredifinedWebElements()[0].getAttributeBy(),"name", "Incorrect first element BY");
				//Assert.assertEquals(getPredifinedWebElements()[0].getAttributeValue(),"fname", "Incorrect first element VALUE");		
				Assert.assertEquals(getPredifinedWebElements().length,20, "Incorrect num of elements");
			}
		};
	}
